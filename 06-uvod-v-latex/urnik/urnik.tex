...
\[\text{velikost tabele}=(\text{konec dneva}-\text{začetek dneva})\cdot\text{število dni}=(20-7)\cdot5=65\]
...
V s plošnem ne zahtevamo, da urnik zadošča vsem šibkim omejitvam, ker je to praktično neizvedljivo.
Res da pa so prav šibke omejitve tiste, ki določajo kakovost urnika, in dober algoritem bo
poskrbel, da se bo zadostilo večini le-teh. Naštejmo nekaj izmed njih:

% V saka od vrstic v okolju `itemize` je en element naštevanja.
\begin{itemize}
...
omejitve so bile kršene ter kolikokrat se kršitev pojavi. S \(x\) označimo število
kršitev omejitve \(a\), z \(y\) pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
\[k=frac{a}{x\cdot y}\]
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   \(n\)-tem koraku označimo s \(Bn\) .

   Za vse proste termine \(T\) izračunamo ceno vstavljanja
   \(x\), ki ga sestavlja \(P\), profesor, ki ta predmet predava,
   ...
   vstavljanja, da bomo \(An\) na koncu vstavili v termin z najboljšo
   ceno vstavljanja.

   Ko preverimo vse proste termine vseh predavalnic, vstavimo \(Bn\)
...
Naj bo \(x\) ter \(k\) sprememba urnika, ki nas je pripeljala v lokalni maksimum. To
spremembo (npr. zamenjava \(A1\) in \(A2\)) sedaj shranimo v
seznam tabujev. V naslednjem koraku izvedemo spremembo \(x\), ki se prav tako shrani v
...
se sprememba ??????????????iskreno preveč je teh vprašajev, da bi jih smiselno zapolnil??????????????????, ki vodi do lokalnega maksimuma, šele tedaj izbriše iz seznama
